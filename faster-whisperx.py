...
model_name = os.getenv("ASR_MODEL", "large-v2")
DEVICE = "cpu"
if torch.cuda.is_available():
    DEVICE = "cuda"
    print("using CUDA")
    model = faster_whisper.WhisperModel(model_name, device="cuda", compute_type="int8")
else:
    model = faster_whisper.WhisperModel(model_name, compute_type="int8")
model_lock = Lock()
...
def run_asr(
    ...
    # )
    # result_aligned = whisperx.align(trans, model_a, metadata, audio, DEVICE)["segments"]
    return trans
